...
/// Custom switch toggle style using MacShield Teal for consistent appearance.
    ...
    func makeBody(configuration: Configuration) -> some View {
        HStack {
            ...
            ZStack(alignment: configuration.isOn ? .trailing : .leading) {
                Capsule()
                    .fill(configuration.isOn ? MacShieldColors.gold : Color(nsColor: .separatorColor))
...
extension ToggleStyle where Self == GoldSwitchStyle {
    /// MacShield Teal switch style — standard size.
    static var goldSwitch: GoldSwitchStyle { GoldSwitchStyle() }

    /// MacShield Teal switch style — compact size for popovers and tight layouts.
