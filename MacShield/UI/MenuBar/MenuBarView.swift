import SwiftUI

/// SwiftUI content for the menu bar popover dropdown.
struct MenuBarView: View {
    let onToggleProtection: () -> Void
    let onSettingsClicked: () -> Void
    let onQuitClicked: () -> Void

    @State private var isProtectionEnabled = Defaults.shared.appSettings.isProtectionEnabled
    @State private var isBlurEnabled = Defaults.shared.appSettings.isBlurEnabled

    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Header
            HStack {
                Image(systemName: "lock.fill")
                    .font(.system(size: 14, weight: .semibold))
                    .foregroundColor(MacShieldColors.gold)
                Text("MacShield")
                    .font(MacShieldTypography.headline)
                Spacer()
            }
            .padding(.horizontal, 16)
            .padding(.top, 14)
            .padding(.bottom, 10)

            Divider()
                .padding(.horizontal, 8)

            // Status + Quick Toggle
            HStack {
                Circle()
                    .fill(isProtectionEnabled ? MacShieldColors.success : MacShieldColors.textSecondary)
                    .frame(width: 8, height: 8)
                Text(isProtectionEnabled ? "Protection Active" : "Protection Off")
                    .font(MacShieldTypography.body)
                    .foregroundColor(.secondary)
                Spacer()
                Toggle("", isOn: $isProtectionEnabled)
                    .toggleStyle(.goldSwitchSmall)
                    .labelsHidden()
                    .onChange(of: isProtectionEnabled) { _ in
                        onToggleProtection()
                    }
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 10)

            // Protected apps count
            let appCount = ProtectedAppsManager.shared.apps.filter(\.isEnabled).count
            HStack(spacing: 6) {
                Image(systemName: "app.badge.checkmark")
                    .font(.system(size: 11))
                    .foregroundColor(.secondary)
                Text("\(appCount) protected app\(appCount == 1 ? "" : "s")")
                    .font(MacShieldTypography.caption)
                    .foregroundColor(.secondary)
            }
            .padding(.horizontal, 16)
            .padding(.bottom, 10)

            // Chat Blur toggle
            HStack {
                Circle()
                    .fill(isBlurEnabled ? MacShieldColors.gold : MacShieldColors.textSecondary)
                    .frame(width: 8, height: 8)
                Text(isBlurEnabled ? "Chat Blur On" : "Chat Blur Off")
                    .font(MacShieldTypography.body)
                    .foregroundColor(.secondary)
                Spacer()
                Toggle("", isOn: $isBlurEnabled)
                    .toggleStyle(.goldSwitchSmall)
                    .labelsHidden()
                    .onChange(of: isBlurEnabled) { newValue in
                        BlurOverlayService.shared.isBlurActive = newValue
                        var settings = Defaults.shared.appSettings
                        settings.isBlurEnabled = newValue
                        Defaults.shared.appSettings = settings
                    }
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 8)

            Divider()
                .padding(.horizontal, 8)

            // Actions
            MenuBarButton(title: "Settings...", icon: "gearshape") {
                onSettingsClicked()
            }

            MenuBarButton(title: "About MacShield", icon: "info.circle") {
                showAboutWindow()
            }

            Divider()
                .padding(.horizontal, 8)

            MenuBarButton(title: "Quit MacShield", icon: "power") {
                onQuitClicked()
            }

            Spacer()
                .frame(height: 8)
        }
        .frame(width: 260)
    }

    private func showAboutWindow() {
        let window = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 360, height: 380),
            styleMask: [.titled, .closable],
            backing: .buffered,
            defer: false
        )
        window.title = "About MacShield"
        window.contentView = NSHostingView(rootView: AboutView())
        window.isReleasedWhenClosed = true
        window.center()
        window.makeKeyAndOrderFront(nil)
        NSApp.activate(ignoringOtherApps: true)
    }
}

// MARK: - Menu Bar Button

private struct MenuBarButton: View {
    let title: String
    let icon: String
    let action: () -> Void

    @State private var isHovered = false

    var body: some View {
        Button(action: action) {
            HStack(spacing: 10) {
                Image(systemName: icon)
                    .font(.system(size: 12))
                    .frame(width: 16)
                Text(title)
                    .font(MacShieldTypography.body)
                Spacer()
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 8)
            .background(
                RoundedRectangle(cornerRadius: 4)
                    .fill(isHovered ? Color.primary.opacity(0.1) : Color.clear)
            )
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
        .onHover { hovering in
            isHovered = hovering
        }
        .padding(.horizontal, 4)
    }
}
