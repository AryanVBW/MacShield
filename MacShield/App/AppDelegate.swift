...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        SafetyManager.shared.onPanicKeyPressed = { [weak self] in
            ...
            BlurOverlayService.shared.dismissAll()
        ...
        // Initialize Chat Blur service (loads settings and starts monitoring if enabled)
        BlurOverlayService.shared.loadSettings()
        ...
        for app in apps {
            if app.autoClose && !SafetyManager.isBlacklisted(app.bundleIdentifier) {
                if let running = NSWorkspace.shared.runningApplications.first(where: {
                    ...
                    NSLog("[MacShield] Auto-closed: %@ (%@)", app.name, app.bundleIdentifier)
                }
    ...
    private func terminateAutoCloseApps() {
        ...
        for app in autoCloseApps {
            ...
            if let running = NSWorkspace.shared.runningApplications.first(where: {
                ...
                NSLog("[MacShield] Auto-closed on wake: %@ (%@)", app.name, app.bundleIdentifier)
            }
        }
    }
    ...
    private static func sendNotification(title: String, body: String) {
        ...
        let request = UNNotificationRequest(
            identifier: "macshield-\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
