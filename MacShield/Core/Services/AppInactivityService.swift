...
final class AppInactivityService: ObservableObject {
    ...
    func startMonitoring() {
        ...
        isMonitoring = true
        NSLog("[MacShield] AppInactivityService started monitoring")
    }
    ...
    func stopMonitoring() {
        ...
        isMonitoring = false
        NSLog("[MacShield] AppInactivityService stopped monitoring")
    }
    ...
    private func startCloseTimer(for bundleID: String) {
        ...
        NSLog("[MacShield] Auto-close timer started for %@ (%.0f min)", bundleID, timeout / 60)
    }
    ...
    private func terminateApp(_ bundleID: String) {
        ...
        guard !SafetyManager.isBlacklisted(bundleID) else {
            NSLog("[MacShield] Refusing to auto-close blacklisted app: %@", bundleID)
            return
        }

        guard let app = NSWorkspace.shared.runningApplications.first(where: { $0.bundleIdentifier == bundleID }) else {
            NSLog("[MacShield] App already closed: %@", bundleID)
            return
        }

        let name = app.localizedName ?? bundleID
        app.terminate()
        NSLog("[MacShield] Auto-closed inactive app: %@ (%@)", name, bundleID)
